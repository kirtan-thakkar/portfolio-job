    ...
    <div
      ...
      {showDays && (
        <div
          ...
          style={{ top: showMonths ? MONTH_LABEL_HEIGHT : 0, width: DAY_LABEL_WIDTH }}>
        ...
        {weeks.map((week, col) =>
          week.days.map((day, row) =>
            day === null ? (
              ...
              <div
                key={`${col}-${row}`}
                className={cn("rounded-[2px]", showCounts && "flex items-center justify-center")}
                style={{
                  gridRow: row + 1,
                  gridColumn: col + 1,
                  backgroundColor: colors[LEVEL_COLORS[day.level]],
                  width: cellSize,
                  height: cellSize,
                }}
                data-date={day.date}
      ...
      <div
        ...
        <div
          className="relative rounded-sm bg-foreground px-2 py-1 text-[10px] text-background whitespace-nowrap">
          {tooltipData && (
            <>
